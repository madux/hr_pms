...
class ImportRecords(models.TransientModel):
    ...
    def import_records_action(self):
        ...
        def create_employee(vals):
            ...
            _logger.info(f'Employee record created...{employee_id.name} and staff {employee_id.employee_number}')
        ...
        if self.import_type == "employee":
            for row in file_data:
                ...
                if find_existing_employee(row[1]):
                    ...
                    appt_date = None 
                    if row[1] in ['', False]:
                        unsuccess_records.append(f'Staff Number is empty at line {str(row[0])}')
                    else:
                        if row[14]:
